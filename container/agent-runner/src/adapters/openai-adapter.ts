...
function resolveOpenRouterReasoningEffort(
  ...
  const disableForTools = parseBooleanEnvVar('OPENROUTER_DISABLE_REASONING_FOR_TOOLS');
  if (hasTools && disableForTools === true) return undefined;

  const override = (process.env.OPENROUTER_REASONING_EFFORT || '').trim().toLowerCase();
...
function resolveToolChoice(
  ...
  const override = (process.env.OPENROUTER_TOOL_CHOICE || '').trim().toLowerCase();
...
function resolveOpenRouterProviderOptions(
  ...
  const sortEnv = (process.env.OPENROUTER_PROVIDER_SORT || 'latency').trim().toLowerCase();
  ...
  const preferredMaxLatency = parsePositiveNumberEnvVar('OPENROUTER_PREFERRED_MAX_LATENCY');
  const preferredMinThroughput = parsePositiveNumberEnvVar('OPENROUTER_PREFERRED_MIN_THROUGHPUT');
  ...
  if (preferredMaxLatency !== undefined) provider.preferred_max_latency = preferredMaxLatency;
  if (preferredMinThroughput !== undefined) provider.preferred_min_throughput = preferredMinThroughput;
...
export class OpenAIAdapter implements ProviderAdapter {
  async *run(input: AdapterInput): AsyncGenerator<AgentEvent> {
    ...
    const baseReasoningEffort = resolveReasoningEffort(input.enableThinking);
    const reasoningEffort = resolveOpenRouterReasoningEffort(baseReasoningEffort, isOpenRouter, hasTools);
    ...
    const openRouterProvider = resolveOpenRouterProviderOptions(isOpenRouter, hasTools);
    const parallelToolCalls = resolveOpenRouterParallelToolCalls(isOpenRouter, hasTools);
    const openRouterDebug = resolveOpenRouterDebugOptions(isOpenRouter);
